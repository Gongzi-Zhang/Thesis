...
Note that for medium and heavy nuclei, the Born approximation,
...
system composed of Z charge-less protons and N neutrons. Now we have a simplified equation
...
For the infinite system, density, instead of $A$, will be a better choice to parameterize
...
\end{equation}
But this tells us only the symmetry energy at nuclear density ($1.22 \times 10^{44}\mathrm{m}^{-3}$).
What about the symmetry energy at other density values? Especially at the nuclear
saturation density ($\sim 1.7 \times 10^{44}\mathrm{m}^{-3}$)? And what about its density
...
physical system, by applying the proper symmetry requirements, one can derive 
...
and far-reaching research. Physicists are doing their best to prove or falsify various
...
If the parity is not conserved, there will be difference between the two scattering 
...
itself into a proton, therefore it is coupling of two current:
\begin{equation}
...
\end{equation}
For right-handed singlets, they don't couple to weak vector bosons, 
...
Because the GSW model preserves the SU(2) structure we talked about before, 
...
\end{equation}
So we come to a striking prediction of the GSW model: the neutral weak interaction.
